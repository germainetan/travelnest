...
def reject_booking():
    ...
    if request.is_json:
        try:
            check_booking = request.get_json()
            print("\nReceived reject_booking in JSON:", check_booking)

            # do the actual work
            # 1. Send order info {cart items}
            result = processrejectBooking(check_booking)
            return jsonify(result), result["code"]
...
def processrejectBooking(check_booking):
    ...
    # Retrieve paymentID, renterID, ownerID
    print('\n-----Invoking Booking microservice-----')
    update_booking = invoke_http(booking_URL + f"{bookingid}?booking_status=rejected", method='PUT')
    booking_result = invoke_http(booking_URL + f"{bookingid}", method='GET')
    renterid = booking_result["renterID"]
    paymentid = booking_result["paymentID"]
    ownerid = booking_result['ownerID']
    ...
    # Get authorizationID from paymentID
    print('\n-----Invoking Payment microservice-----')
    authorizationid_result = invoke_http(payment_URL + f"{paymentid}", method='GET')
    authorizationid = authorizationid_result["authorizationID"]
    print('captureid:', authorizationid)

    # Invoke the Payment microservice again
    # Reject booking & void payment
    print('\n-----Invoking Payment microservice-----')
    booking_rejected_result = invoke_http(payment_URL + f"api/orders/{authorizationid}/authorization/void", method='POST')
    print('booking_rejected_result:', booking_rejected_result)
    ...
    return {
        "code": 200,
        "data": "Booking Rejection Successful"
    }
